        ...
        db.query("SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME = 'lanzaderas';", (err, res) => {
            if (res.rowCount == 0) {
                db.query("CREATE TABLE IF NOT EXISTS clyzer.lanzaderas (id serial, nombre text, combustible text, pais text, actividad text, PRIMARY KEY (id));", (err) => {
                    ...
                    db.query("INSERT INTO clyzer.lanzaderas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'Transbordador espacial', 'Sólido + Liquido', 'EEUU', '1981-2011');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
                    ...
                    db.query("INSERT INTO clyzer.lanzaderas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'Energía', 'Liquido', 'Rusia/Ucrania', '1987-1988');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
                    db.query("INSERT INTO clyzer.lanzaderas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'Protón', 'Liquido', 'Rusia/Ucrania', '2001-Act');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
                    ...
                    db.query("INSERT INTO clyzer.lanzaderas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'Ariane V', 'Sólido + Liquido', 'Europa', '1996-Act');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
                    // Agregado de datos de lanzaderas de Japón
                    db.query("INSERT INTO clyzer.lanzaderas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'H-IIA', 'Sólido + Liquido', 'Japón', '2001-Act');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
        ...
        db.query("SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME = 'notripuladas';", (err, res) => {
            if (res.rowCount == 0) {
                db.query("CREATE TABLE IF NOT EXISTS clyzer.notripuladas (id serial, nombre text, combustible text, pais text, actividad text, PRIMARY KEY (id));", (err) => {
                    ...
                    db.query("INSERT INTO clyzer.notripuladas(id, nombre, combustible, pais, actividad) VALUES (DEFAULT, 'Surveyor', 'Sólido + Liquido', 'EEUU', '1966-Act');", (err) => {
                        if (err){
                            console.log(err.stack)
                        }
                    })
